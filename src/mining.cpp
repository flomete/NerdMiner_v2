...
void runMonitor(void *name){
  ...
  while(1){
    ...
    /*Serial.printf(">>>mElapsed %d Mhashes %d templates, %d halfshares %d hashes %d hashes\n",
      mElapsed, Mhashes, templates, halfshares, hashes);*/
    ...
    if((lastHour < hr && (hr % 12) == 0) || forceRestart){
      resetESP(mElapsed);
    }
    ...
    render.rdrawString(String(temp).c_str(), 278, 5, 0xDEDB);

    render.setFontSize(7);
    render.rdrawString(String(0).c_str(), 285, 5, 0xDEDB);
...
long getLastData(){
  preferences.begin("Nerd-Miner", false);
  Mhashes = preferences.getULong("Mhashes", 0);
  templates = preferences.getULong("templates", 0);
  halfshares = preferences.getUInt("halfshares", 0);
  shares = preferences.getUInt("shares", 0);
  valids = preferences.getUInt("valids", 0);
  hashes = preferences.getULong("hashes", 0);
  long time = preferences.getULong("time", 0);
  unsigned long secElapsed=time/1000;
  lastHour = secElapsed/3600;
  preferences.clear();
  return time; 
}
void resetESP(long time){
  preferences.putULong("Mhashes", Mhashes);
  preferences.putULong("templates", templates);
  preferences.putUInt("halfshares", halfshares);
  preferences.putUInt("shares", shares);
  preferences.putUInt("valids", valids);
  preferences.putULong("hashes", hashes);
